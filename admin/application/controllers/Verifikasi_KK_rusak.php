...
class Verifikasi_KK_rusak extends Guide
{
    ...
    public function index()
    {
        ...
        $data['data'] = $this->db->get('kk_rusak')->result_array();
    ...
    public function proses($param = "")
    {
        ...
        $data['data'] = $this->db->get('kk_rusak')->row_array();
    ...
    public function verif($id = "")
    {
        ...
        if ($this->db->update('kk_rusak')) {
            $this->db->where('id', $id);
            $user = $this->db->get('kk_rusak')->row_array();
            $qrval = $user['nama'] . ',' . $user['email'] . ',' . $user['nohp'] . ',' . 'sa7d4c44a3ajads6ddd445ca0d1b65ca' . ',' . 'kk' . ',' . $id;
            $this->sendEmail(['email' => $user['email'], 'message' => "Pengajuan KK Rusak anda telah diverifikasi oleh petugas, Berkas akan segera dikirim oleh petugas ke alamat anda.", 'qrval' => $qrval, 'istolak' => true]);
