                ...
                <tbody>
                    ...
                    <?php foreach ($data as $row) : ?>
                        <tr>
                            ...
                            <td>
                                <a href="" class="btn btn-warning btn-sm btn-sm editModal" data-id="<?= $row['pk_setup_perjaman_mesin_generator']; ?>" data-toggle="modal" data-target="#editModal"><span class="fa fa-edit"></span></a>&nbsp;
                                <a href="<?= base_url('setup_perjaman_mesin_generator/delete/') . $row['pk_setup_perjaman_mesin_generator']; ?>" class="btn btn-danger btn-sm btn-sm" onclick="javascript: return confirm('Anda yakin ingin menghapus?')"><span class="fa fa-trash"></span></a>
                            </td>
                        </tr>
                        <?php $i++; ?>
                    <?php endforeach; ?>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Setup Perjaman Mesin Generator</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form action="<?= base_url('setup_perjaman_mesin_generator/add'); ?>" method="POST">
                <div id="hidden_field">
                    <input type="hidden" name="input[pk_setup_perjaman_mesin_generator]" value="<?= 'SPM' . date('dhs'); ?>">
...
<div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="<?= base_url('setup_perjaman_mesin_generator/edit'); ?>" method="POST">
                <div id="hidden_field">
                    <input type="hidden" id="pk_setup_perjaman_mesin_generator" name="pk[pk_setup_perjaman_mesin_generator]">
...
<script>
    $(document).ready(function() {
        ...
        $('.editModal').on('click', function() {
            $('#pk_setup_perjaman_mesin_generator, #mesin_sulzer, #tanggal_input').val(''); //clear

            var id = $(this).data('id');
            console.log(id);

            $.ajax({
                url: "<?= base_url('setup_perjaman_mesin_generator/ajax'); ?>",
                method: 'GET',
                data: {
                    data: id
                },
                ...
                success: function(data) { //menampilkan
                    ...
                    $('#pk_setup_perjaman_mesin_generator').val(hasil.pk_setup_perjaman_mesin_generator);
