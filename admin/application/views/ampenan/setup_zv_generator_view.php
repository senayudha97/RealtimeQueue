                ...
                <tbody>
                    ...
                    <?php foreach ($data as $row) : ?>
                        <tr>
                            ...
                            <td><?= $row['pltd']; ?></td>
                            <td>
                                <div class="btn btn-success btn-sm btn-sm mesin" data-id="<?= $row['pk_setup_zv_generator']; ?>" data-toggle="modal" data-target="#detailModalMesin"><span class="fa fa-info-circle"> &nbsp;</span>Lihat Detail</div>
                            </td>
                            <td>
                                <div class="btn btn-success btn-sm btn-sm generator" data-id="<?= $row['pk_setup_zv_generator']; ?>" data-toggle="modal" data-target="#detailModalGenerator"><span class="fa fa-info-circle"> &nbsp;</span>Lihat Detail</div>
                            </td>
                            <td><?= $row['jam_kerja_mesin_awal']; ?></td>
                            <td><?= $row['jam_kerja_mesin_akhir']; ?></td>
                            <td>
                                <a href="" class="btn btn-warning btn-sm btn-sm editModal" data-id="<?= $row['pk_setup_zv_generator']; ?>" data-toggle="modal" data-target="#editModal"><span class="fa fa-edit"></span></a>&nbsp;
                                <a href="<?= base_url('setup_zv_generator/delete/') . $row['pk_setup_zv_generator']; ?>" class="btn btn-danger btn-sm btn-sm" onclick="javascript: return confirm('Anda yakin ingin menghapus?')"><span class="fa fa-trash"></span></a>
                            </td>
                        </tr>
                        <?php $i++; ?>
                    <?php endforeach; ?>
...
<div class="modal fade" id="addModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="<?= base_url('setup_zv_generator/add'); ?>" method="POST">
                <div id="hidden_field">
                    <input type="hidden" name="input[pk_setup_zv_generator]" value="<?= 'SZG' . date('dhs'); ?>">
...
<div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="<?= base_url('setup_zv_generator/edit'); ?>" method="POST">
                <div id="hidden_field">
                    <input type="hidden" name="pk[pk_setup_zv_generator]" id="pk_setup_zv_generator">
...
<script>
    $(document).ready(function() {
        ...
        $('.mesin').on('click', function() {
            ...
            $.ajax({
                url: "<?= base_url('setup_zv_generator/ajax'); ?>",
                method: 'GET',
                data: {
                    data: id
                },
        ...
        $('.editModal').on('click', function() {
            ...
            $.ajax({
                ...
                success: function(data) { //menampilkan
                    var hasil = JSON.parse(data);
                    $('#pk_setup_zv_generator').val(hasil.pk_setup_zv_generator);
